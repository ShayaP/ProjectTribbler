...
func TestTimeToGetDisk(t *testing.T) {
	...
	fronts := []string{frontAddr0, frontAddr1}
	...
	for i := 0; i < num_samples; i++ {
		...
		time_nanos := performance_check.CheckTimeToGetDisk(t, client0, client1)
		total += time_nanos
	}

	// Average the results
	t.Logf("Average for calling Get with the disk: %d", total/int64(num_samples))
}
